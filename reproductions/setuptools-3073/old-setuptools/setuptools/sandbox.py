...
def setup_context(setup_dir):
    ...
    with save_pkg_resources_state(), save_modules():
        hide_setuptools()
        with save_path(), save_argv(), override_temp(temp_dir), pushd(setup_dir):
...
def run_setup(setup_script, args):
    ...
    with setup_context(setup_dir):
        try:
            ...
            dunder_file = (
                setup_script if isinstance(setup_script, str) else setup_script.encode(sys.getfilesystemencoding())
            )
...
WRITE_FLAGS = functools.reduce(
    operator.or_, [getattr(_os, a, 0) for a in ["O_WRONLY", "O_RDWR", "O_APPEND", "O_CREAT", "O_TRUNC", "O_TEMPORARY"]]
)
