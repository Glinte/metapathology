    ...
    def open_url(self, url, warning=None):
        ...
        try:
            ...
            if warning:
                ...
                raise DistutilsError("%s returned a bad status line. The server might be down, %s" % (url, v.line))
        except (OSError, http_client.HTTPException) as v:
            if warning:
                self.warn(warning, v)
            else:
...
def htmldecode(text):
    ...
    >>> htmldecode('https://../package_name-0.1.2.tar.gz?tokena=A&amp;tokenb=B">package_name-0.1.2.tar.gz')
