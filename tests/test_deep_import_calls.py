"""Deep ``builtins.__import__`` observation, including cache-hit imports."""
...
def test_cache_hits_are_observed_and_restored(run_python: RunPython, tmp_path: Path) -> None:
    (tmp_path / "cache_target.py").write_text("VALUE = 1\n", encoding="utf-8")

    proc = run_python(CACHE_HIT)
...
def test_fromlist_and_relative_level_are_captured(run_python: RunPython, tmp_path: Path) -> None:
    ...
    proc = run_python(FROMLIST_AND_LEVEL)
...
def test_failed_import_records_the_raised_outcome(run_python: RunPython, tmp_path: Path) -> None:
    proc = run_python(RAISED)
...
def test_wrapping_is_chain_safe_with_other_import_wrappers(run_python: RunPython, tmp_path: Path) -> None:
    proc = run_python(CHAIN_SAFE)
...
assert monitor.deep_import_calls_status == "disabled"
print("OK")
"""


def test_mechanism_is_off_unless_requested(run_python: RunPython, tmp_path: Path) -> None:
    proc = run_python(DISABLED_BY_DEFAULT)
