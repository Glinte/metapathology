...
def test_record_fields_are_read_only_and_slotted() -> None:
    record = MonitoringError(sequence=1, where="test", exception_type_name="ValueError")

    assert record.sequence == 1
    assert not hasattr(record, "__dict__")
    with pytest.raises(AttributeError, match="read-only"):
        setattr(record, "sequence", 2)


def test_records_keep_repr_without_generated_equality_or_pattern_matching() -> None:
    first = MonitoringError(sequence=1, where="test", exception_type_name="ValueError")
    second = MonitoringError(sequence=1, where="test", exception_type_name="ValueError")

    assert repr(first) == ("MonitoringError(sequence=1, where='test', exception_type_name='ValueError', message=None)")
...
def test_object_reference_is_plain_read_only_identity_data() -> None:
    reference = ObjectIdentity(object_id=42, type_name="function", name="path_hook_for_FileFinder")

    assert repr(reference) == ("ObjectIdentity(object_id=42, type_name='function', name='path_hook_for_FileFinder')")
...
def test_object_ref_of_captures_function_name_but_not_plain_object_name() -> None:
    ...
    hook_ref = ObjectIdentity.of(path_hook_for_FileFinder)
    ...
    plain = object()
    plain_ref = ObjectIdentity.of(plain)
...
def test_object_ref_of_does_not_dispatch_to_a_spoofed_class_attribute() -> None:
    ...
    hostile = Hostile()
    reference = ObjectIdentity.of(hostile)
...
def test_importer_cache_values_distinguish_negative_entries_and_replacements() -> None:
    finder = ObjectIdentity(object_id=42, type_name="FileFinder")
...
def test_import_search_started_contains_only_plain_capture_data() -> None:
    record = ImportSearchStarted(
        sequence=7,
        search_id=3,
    ...
    assert repr(record) == (
        "ImportSearchStarted(sequence=7, search_id=3, fullname='example.module', meta_path_id=41, "
