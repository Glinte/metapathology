...
def _run(python_runner: PythonRunner, retained_result: str, body: str) -> "subprocess.CompletedProcess[str]":
    code = f"RETAINED_RESULT = {retained_result!r}\n" + SCENARIO + body
    return python_runner.run_code_ok(code)
...
def test_displaced_finder_that_still_resolves_is_reported(python_runner: PythonRunner) -> None:
    proc = _run(python_runner, "spec", RETURNS_SPEC)
...
def test_displaced_finder_that_no_longer_resolves_is_reported(python_runner: PythonRunner) -> None:
    proc = _run(python_runner, "none", RETURNS_NONE)
...
def test_replayed_finder_that_raises_cannot_break_the_report(python_runner: PythonRunner) -> None:
    proc = _run(python_runner, "raise", RAISES)
...
def test_reload_target_lookups_are_declined_not_answered_wrongly(python_runner: PythonRunner) -> None:
    proc = _run(python_runner, "spec", DECLINES_TARGET)
...
def test_replay_is_capped_and_reports_the_overflow(python_runner: PythonRunner) -> None:
    proc = _run(python_runner, "spec", PROBE_CAP)
...
def test_repeated_reports_do_not_grow_the_event_log(python_runner: PythonRunner) -> None:
    proc = _run(python_runner, "spec", REPEAT_REPORTS_DO_NOT_GROW_EVENTS)
...
def test_replay_performs_no_foreign_calls_unless_requested(python_runner: PythonRunner) -> None:
