"""Isolated child scenarios extracted from tests/monitoring/test_unsafe_import_branch_exploration.py."""

import sys

_scenario = sys.argv.pop(1)

if _scenario == "meta_path_exploration":
    ...
    mechanism = next(
        item for item in document["capture"]["mechanisms"] if item["name"] == "unsafe_import_branch_exploration"
    )
    assert mechanism["enabled"] is True
    events = [
        item
        for item in document["timeline"]
        if item["kind"] == "import_branch_exploration_call" and item["data"]["fullname"] == "unsafe_meta_target"
    ]
    ...
    results = [
        ...
        if item["kind"] == "import_branch_exploration_result" and item["module"] == "unsafe_meta_target"
    ]
    ...
    assert any(
        comparison["left_result_ref"] in explored_result_ids or comparison["right_result_ref"] in explored_result_ids
...
elif _scenario == "path_hook_exploration":
    import importlib.machinery
    import json
    import sys
    ...
    events = [
        item["data"]
        for item in document["timeline"]
        if item["kind"] == "import_branch_exploration_call" and item["data"]["fullname"] == "unsafe_path_target"
    ]
    assert any(
...
elif _scenario == "partial_coverage":
    import sys
    import threading
    ...
    assert monitor.unsafe_import_branch_exploration_status == "partial_capture_prerequisites_disabled"
    metapathology.uninstall()
else:
