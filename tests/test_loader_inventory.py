...
records = {
    module["name"]: (group["loader"], module)
    for group in groups
...
def test_loader_inventory_groups_safe_module_metadata(run_python: RunPython, tmp_path: Path) -> None:
    ...
    proc = run_python(INVENTORY, str(lazy_source), str(archive))
